...
test('reports liveness and version without product capabilities', async () => {
  ...
  const response = handler(new Request('http://control-plane.test/health/live'))
...
test('reports not ready while dependencies are unavailable', async () => {
  ...
  const response = handler(new Request('http://control-plane.test/health/ready'))
...
test('publishes an intentionally empty versioned status contract', async () => {
  ...
  const response = handler(new Request('http://control-plane.test/internal/v1/status'))
...
test('does not pretend later authentication or organization routes exist', async () => {
  const handler = createControlPlaneHandler()
  const response = handler(new Request('http://control-plane.test/api/organizations'))
